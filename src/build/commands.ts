...
export async function buildCommand(context: ExtensionContext, item?: BuildTreeItem) {
  ...
  return context.buildManager.buildCommand({ debug: false, scheme: item?.scheme });
}
...
export async function debuggingBuildCommand(context: ExtensionContext, item?: BuildTreeItem) {
  ...
  return context.buildManager.buildCommand({ debug: true, scheme: item?.scheme });
}
...
export async function launchCommand(context: ExtensionContext, item?: BuildTreeItem) {
  return context.buildManager.lunchCommand({ debug: false, scheme: item?.scheme });
}
...
export async function runCommand(context: ExtensionContext, item?: BuildTreeItem) {
  ...
  return context.buildManager.runCommand({ debug: false, scheme: item?.scheme });
}

/**
 * Among all destination that run selected scheme, find and stop the running app for that scheme
 */
export async function stopCommand(context: ExtensionContext, item?: BuildTreeItem) {
  context.updateProgressStatus("Stopping running app");
...
export async function debuggingRunCommand(context: ExtensionContext, item?: BuildTreeItem) {
  ...
  return context.buildManager.runCommand({ debug: true, scheme: item?.scheme });
}
...
export async function cleanCommand(context: ExtensionContext, item?: BuildTreeItem) {
  return context.buildManager.cleanCommand({ scheme: item?.scheme });
}

export async function testCommand(context: ExtensionContext, item?: BuildTreeItem) {
  return context.buildManager.testCommand({
    scheme: item?.scheme,
  });
}
